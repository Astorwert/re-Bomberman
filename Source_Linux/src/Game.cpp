...
void Game::loadMenu(const char* title, const int &xpos, const int &ypos, const int &width, const int &height, const int &flags) {
	...
	//icon = IMG_Load("icon.jpg");
	//SDL_SetWindowIcon(win, icon);

	SDL_LoadWAV("src/materials/Powerup.wav", &wavSpec, &wavPowerup, &wavLength);
	SDL_LoadWAV("src/materials/Death.wav", &wavSpec, &wavDeath, &wavLength1);
	SDL_LoadWAV("src/materials/Explosion.wav", &wavSpec, &wavExplosion, &wavLength2);

	tileset = loadTexture("src/materials/Tileset.png", ren);
	menu = loadTexture("src/materials/Menu.jpg", ren);
	bomb = loadTexture("src/materials/Bomb.png", ren);
	backg = loadTexture("src/materials/background.jpg", ren);
	tilesetplayer = loadTexture("src/materials/Man.png", ren);
	tilesetdeath = loadTexture("src/materials/Death.png", ren);
...
void Game::start() {
	...
	fin.open ("src/materials/Map.txt");
	fout.open ("src/materials/Score.txt");
...
void Game::boom(const int &x, const int &y, const int &range, const int &num) {
	...
	SDL_QueueAudio(Channel1, wavExplosion, wavLength2);
...
void Game::death(const int &x, const int &y) {
	SDL_ClearQueuedAudio(Channel1);
	SDL_QueueAudio(Channel1, wavDeath, wavLength1);
	SDL_PauseAudioDevice(Channel1, 0);
